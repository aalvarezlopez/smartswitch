...
static void setMACs (const uint8_t *mac) {
    copyMac(buffer + ETH_DST_MAC, mac);
    copyMac(buffer + ETH_SRC_MAC, mymac);
}

static void setMACandIPs (const uint8_t *mac, const uint8_t *dst) {
    setMACs(mac);
    copyIp(buffer + IP_DST_P, dst);
    copyIp(buffer + IP_SRC_P, myip);
}

#ifdef AAL_DISABLED_CODE
...
void fill_ip_hdr_checksum() {
    ...
    buffer[IP_FLAGS_P] = 0x40; // don't fragment
    buffer[IP_FLAGS_P+1] = 0;  // fragment offset
    buffer[IP_TTL_P] = 64; // ttl
...
void makeUdpReply (const char *data,uint8_t datalen,uint16_t port) {
    if (datalen>220)
        datalen = 220;
...
void clientIcmpRequest(const uint8_t *destip) {
    ...
    gPB[ICMP_IDENT_L_P] = myip[3]; // last byte of my IP
...
void udpTransmit (uint16_t datalen) {
    #if 0 /* FILL WITH LENGTH AND CHKSUM */
    buffer[IP_TOTLEN_H_P] = (IP_HEADER_LEN+UDP_HEADER_LEN+datalen) >> 8;
    buffer[IP_TOTLEN_L_P] = IP_HEADER_LEN+UDP_HEADER_LEN+datalen;
    fill_ip_hdr_checksum();
    buffer[UDP_LEN_H_P] = (UDP_HEADER_LEN+datalen) >>8;
...
void sendUdp (const char *data, uint8_t datalen, uint16_t sport,
                         const uint8_t *dip, uint16_t dport) {
    udpPrepare(sport, dip, dport);
    if (datalen>220)
        datalen = 220;
    memcpy(buffer + UDP_DATA_P, data, datalen);
    udpTransmit(datalen);
}
...
void client_arp_whohas(uint8_t *ip_we_search) {
    ...
    memcpy_P(buffer + ETH_ARP_P, arpreqhdr, sizeof arpreqhdr);
    memset(buffer + ETH_ARP_DST_MAC_P, 0, ETH_LEN);
    copyMac(buffer + ETH_ARP_SRC_MAC_P, mymac);
    copyIp(buffer + ETH_ARP_DST_IP_P, ip_we_search);
    copyIp(buffer + ETH_ARP_SRC_IP_P, myip);
    packetSend(42);
}
...
void browseUrl (const char *urlbuf, const char *urlbuf_varpart, const char *hoststr, void (*callback)(uint8_t,uint16_t,uint16_t)) {
    browseUrl(urlbuf, urlbuf_varpart, hoststr, PSTR("Accept: text/html"), callback);
}

void browseUrl (const char *urlbuf, const char *urlbuf_varpart, const char *hoststr, const char *additionalheaderline, void (*callback)(uint8_t,uint16_t,uint16_t)) {
...
void httpPost (const char *urlbuf, const char *hoststr, const char *additionalheaderline, const char *postval, void (*callback)(uint8_t,uint16_t,uint16_t)) {
    client_urlbuf = urlbuf;
    client_hoststr = hoststr;
    client_additionalheaderline = additionalheaderline;
...
uint8_t packetLoopIcmpCheckReply (const uint8_t *ip_monitoredhost) {
...
void persistTcpConnection(bool persist) {
