...
/*------------------------------------------------------------*/
/* Note that uninitialized variables with _STATIC duration are
...
_STATIC WORD Fsid; /* File system mount ID */
...
#elif _USE_LFN == 1         /* LFN feature with _STATIC working buffer */
_STATIC WCHAR LfnBuf[_MAX_LFN + 1];
#define DEF_NAMEBUF         BYTE sfn[12]
...
)
{
    ...
    if (i == _FS_SHARE) { /* The file is not opened */
        return (be || acc == 2) ? FR_OK :
               FR_TOO_MANY_OPEN_FILES;    /* Is there a blank entry for new file? */
    }
...
_STATIC FRESULT move_window(FATFS* fs, /* File system object */
                          ...
                          ) /* Move to zero only writes back dirty window */
{
    ...
    if (wsect != sector) { /* Changed current window */
        ...
        if (fs->wflag) { /* Write back dirty window if needed */
            if (disk_write(fs->drv, fs->win, wsect, 1) != RES_OK) { return FR_DISK_ERR; }
        ...
        if (sector) {
            if (disk_read(fs->drv, fs->win, sector, 1) != RES_OK) { return FR_DISK_ERR; }
...
/*-----------------------------------------------------------------------*/
#if !_FS_READONLY
_STATIC FRESULT sync( /* FR_OK: successful, FR_DISK_ERR: failed */
    FATFS* fs /* File system object */
)
{
    FRESULT res;
...
_STATIC DWORD clust2sect( /* !=0: Sector number, 0: Failed - invalid cluster# */
    FATFS* fs, /* File system object */
...
_STATIC FRESULT put_fat(FATFS* fs, /* File system object */
...
/*-----------------------------------------------------------------------*/
#if !_FS_READONLY
_STATIC FRESULT remove_chain(FATFS* fs, /* File system object */
...
_STATIC FRESULT dir_sdi(DIR* dj, /* Pointer to directory object */
                      ...
                      )
{
    ...
    if (clst == 0) { /* Static table (root-dir in FAT12/16) */
        ...
        while (idx >= ic) { /* Follow cluster chain */
            ...
            if (clst < 2 ||
                clst >= dj->fs->n_fatent) { /* Reached to end of table or int error */
                return FR_INT_ERR;
            }
    ...
    dj->dir = dj->fs->win + (idx % (SS(dj->fs) / SZ_DIR)) *
              SZ_DIR; /* Ptr to the entry in the sector */
...
)
{
    ...
    if (!(i % (SS(dj->fs) / SZ_DIR))) { /* Sector changed? */
        ...
        if (dj->clust == 0) { /* Static table */
            ...
            if (((i / (SS(dj->fs) / SZ_DIR)) & (dj->fs->csize - 1)) ==
                0) { /* Cluster changed? */
...
_STATIC FRESULT dir_find(DIR
                        *dj /* Pointer to the directory object linked to the file name */
                       )
{
    FRESULT res;
    BYTE c, *dir;
...
/*-----------------------------------------------------------------------*/
#if _FS_MINIMIZE <= 1
_STATIC FRESULT dir_read(DIR
                        *dj /* Pointer to the directory object that pointing the entry to be read */
                       )
{
    FRESULT res;
    BYTE c, *dir;
    ...
    while (dj->sect) {
        ...
        if (c != DDE && (_FS_RPATH || c != '.') &&
            !(dir[DIR_Attr] & AM_VOL)) { /* Is it a valid entry? */
            break;
        }
...
)
{
    ...
    if (sn[NS] &
        NS_LFN) { /* When LFN is to be created, reserve an SFN + LFN entries. */
        for (ne = 0; lfn[ne]; ne++);
...
_STATIC FRESULT dir_remove( /* FR_OK: Successful, FR_DISK_ERR: A disk error */
...
_STATIC FRESULT create_name(DIR* dj, /* Pointer to the directory object */
                           const TCHAR** path /* Pointer to pointer to the segment in the path string */
                          )
{
#ifdef _EXCVT
    _STATIC const BYTE excvt[] =
        _EXCVT; /* Upper conversion table for extended chars */
#endif
    ...
    for (;;) {
        ...
        if (w < 0x80 &&
            chk_chr("\"*:<>\?|\x7F", w)) { /* Reject illegal chars for LFN */
            return FR_INVALID_NAME;
        }
    ...
    *path = &p[si]; /* Return pointer to the next segment */
    ...
    for (si = 0; lfn[si] == ' ' ||
         lfn[si] == '.'; si++); /* Strip leading spaces and dots */
    ...
    if (!(cf&
          NS_LFN)) { /* When LFN is in 8.3 format without extended char, NT flags are created */
...
_STATIC FRESULT follow_path( /* FR_OK(0): successful, !=0: error code */
...
_STATIC BYTE
check_fs( /* 0:FAT-VBR, 1:Valid BR but not FAT, 2:Not a BR, 3:Disk error */
    FATFS* fs, /* File system object */
...
_STATIC FRESULT chk_mounted( /* FR_OK(0): successful, !=0: any error occurred */
...
)
{
    ...
    *rfs = fs =
               FatFs[vol]; /* Return pointer to the corresponding file system object */
    ...
    if (fs->fs_type) { /* If the logical drive has been mounted */
        ...
        if (!(stat&
              STA_NOINIT)) { /* and the physical drive is kept initialized (has not been changed), */
            if (!_FS_READONLY && chk_wp &&
    ...
    if (disk_ioctl(fs->drv, GET_SECTOR_SIZE, &fs->ssize) != RES_OK) { return FR_DISK_ERR; }
    ...
    if (LD_WORD(fs->win + BPB_BytsPerSec) != SS(
            fs)) { /* (BPB_BytsPerSec must be equal to the physical sector size) */
    ...
    if (fmt == FS_FAT32) {
        ...
        fs->dirbase = LD_DWORD(fs->win +
                               BPB_RootClus); /* Root directory start cluster */
    ...
    if (fs->fsize < (szbfat + (SS(fs) - 1)) / SS(
            fs)) { /* (BPB_FATSz must not be less than required) */
        return FR_NO_FILESYSTEM;
    }
...
_STATIC FRESULT validate( /* FR_OK(0): The object is valid, !=0: Invalid */
...
)
{
    ...
    if (disk_status(fs->drv) & STA_NOINIT) { return FR_NOT_READY; }
              ...
              )
{
    ...
    if (res == FR_OK) { res = follow_path(&dj, path); } /* Follow the file path */
    ...
    if (res == FR_OK) {
        if (mode&
            FA_CREATE_ALWAYS) { /* Set file change flag if created or overwritten */
            mode |= FA__WRITTEN;
        }
    ...
    for (; btr; /* Repeat until all data read */
        ...
        if ((fp->fptr % SS(fp->fs)) == 0) { /* On the sector boundary? */
            ...
            if (cc) { /* Read maximum contiguous sectors directly */
                ...
                if ((fp->flag & FA__DIRTY) && fp->dsect - sect < cc) mem_cpy(
                        rbuff + ((fp->dsect - sect) * SS(fp->fs)), fp->buf,
                        SS(fp->fs));
            ...
            if (fp->dsect != sect) { /* Load data sector if not in cache */
                ...
                if (disk_read(fp->fs->drv, fp->buf, sect,
                              1) != RES_OK) { /* Fill sector cache */
                    ABORT(fp->fs, FR_DISK_ERR);
                }
            }
        ...
        rcnt = SS(fp->fs) - (fp->fptr % SS(
                                 fp->fs)); /* Get partial sector data from sector buffer */
    ...
    for (; btw; /* Repeat until all data written */
        ...
        if ((fp->fptr % SS(fp->fs)) == 0) { /* On the sector boundary? */
            ...
            if (fp->fs->winsect == fp->dsect &&
                move_window(fp->fs, 0)) { /* Write-back sector cache */
                ABORT(fp->fs, FR_DISK_ERR);
            }
        ...
        wcnt = SS(fp->fs) - (fp->fptr % SS(
                                 fp->fs));/* Put partial sector into file I/O buffer */
               ...
               )
{
        ...
        /* Normal Seek */
    {
        ...
        if (ofs) {
            ...
            if (clst != 0) {
                while (ofs > bcs) { /* Cluster following loop */
                    ...
                    if (clst <= 1 || clst >= fp->fs->n_fatent) ABORT(fp->fs,
                                                                         FR_INT_ERR);
        ...
        if (fp->fptr % SS(fp->fs) &&
            ...
            if (disk_read(fp->fs->drv, fp->buf, nsect,
                          1) != RES_OK) { /* Fill sector cache */
                ABORT(fp->fs, FR_DISK_ERR);
            }
                ...
                )
{
    ...
    if (res == FR_OK) {
        ...
        if (_FS_RPATH && res == FR_OK && (dj.fn[NS] & NS_DOT)) res =
                FR_INVALID_NAME; /* Cannot remove dot entry */
               ...
               )
{
    ...
    if (res == FR_OK) {
        ...
        if (_FS_RPATH && res == FR_NO_FILE && (dj.fn[NS] & NS_DOT)) res =
                FR_INVALID_NAME;

        if (res == FR_NO_FILE) { /* Can create a new directory */
            dcl = create_chain(dj.fs,
            ...
            if (res == FR_OK) { /* Initialize the new directory table */
                ...
                if (dj.fs->fs_type == FS_FAT32 && pcl == dj.fs->dirbase) pcl =
                        0;
    ...
    if (res == FR_OK) {
        ...
        if (res == FR_OK) {
            ...
            if (!dir) { /* Is it a root directory? */
                ...
                dir[DIR_Attr] = (value & mask) | (dir[DIR_Attr] & (BYTE)
                ...
                )
{
    ...
    if (res == FR_OK) {
        ...
        if (_FS_RPATH && res == FR_OK && (djo.fn[NS] & NS_DOT)) res =
                FR_INVALID_NAME;
...
)
{
    ...
    *bf = 0; /* Initialize byte counter */
    ...
    for (; btr &&
         (*func)(0, 0); /* Repeat until all data transferred or stream becomes busy */
        ...
        fp->dsect = sect;
        rcnt = SS(fp->fs) - (WORD)(fp->fptr % SS(
                                       fp->fs)); /* Forward data from sector window */
...
{
    _STATIC const WORD vst[] = {1024, 512, 256, 128, 64, 32, 16, 8, 4, 2, 0};
    _STATIC const WORD cst[] = {32768, 16384, 8192, 4096, 2048, 16384, 8192, 4096, 2048, 1024, 512};
    ...
    if (_MULTI_PARTITION && part) {
        ...
        if (sfd) { /* No patition table (SFD) */
            ...
            tbl = fs->win +
                  MBR_Table; /* Create partiton table for single partition in the drive */
            ...
            if (disk_write(pdrv, fs->win, 0,
                           1) != RES_OK) { /* Write it to the MBR sector */
                return FR_DISK_ERR;
            }
    ...
    if (fmt == FS_FAT32) {
        ...
        mem_cpy(tbl + BS_VolLab32, "NO NAME    " "FAT32   ",
        ...
        mem_cpy(tbl + BS_VolLab, "NO NAME    " "FAT     ",
                19); /* Volume label, FAT signature */
    }
    ...
    if (disk_write(pdrv, tbl, b_vol,
                   1) != RES_OK) { /* Write it to the VBR sector */
        return FR_DISK_ERR;
    }
...
)
{
    ...
    for (cc = res = 0; cc != EOF; res += cc) {
        ...
        v = (f & 4) ? (ULONG)va_arg(arp, long) : ((d == 'D') ? (ULONG)(long)va_arg(arp,
                                                                                   int) : (ULONG)va_arg(arp, unsigned int));
